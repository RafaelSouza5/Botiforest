...
module.exports = {
    ...
    run: async(client, message, args) => {
        ...
        message.reply({ embeds: [embed_1] }).then(msg => {
            let coletor_1 = message.channel.createMessageCollector({ filter: msg => msg.author.id == message.author.id, max: 1 });
            ...
            coletor_1.on("collect", (palavra_1) => {
                ...
                if (chat) {
                    message.reply({ embeds: [embed_2] }).then(msg => {
                        ...
                        let coletor_2 = message.channel.createMessageCollector({ filter: msg => msg.author.id == message.author.id, max: 1 });

                        coletor_2.on("collect", (palavra_2) => {

                            let titulo = palavra_2.content;

                             message.reply({ embeds: [embed_3] }).then(msg => {

                                let coletor_3 = message.channel.createMessageCollector({ filter: msg => msg.author.id == message.author.id, max: 1 });

                                coletor_3.on("collect", (palavra_3) => {

                                    let desc = palavra_3.content;

                                    message.reply(`O aviso foi enviado para ${chat} com sucesso.`).then(msg => {
                                        chat.send({ embeds: [
